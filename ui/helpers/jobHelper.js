...
export const findSelectedInstance = function findSelectedInstance() {
  const selectedEl = $('.th-view-content').find(".job-btn.selected-job, .selected-count").first();
...
// Set the jobId job to selected and if scrollTo is true, scroll it into view.
export const selectJobById = function selectJobById(jobId, scrollTo) {
  const job = findJobInstance(jobId, scrollTo);
